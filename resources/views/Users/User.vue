<template>
    <main>
        <AdaptList :items="['one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight8888888', 'nine']">
            <template scope="list">
                <ul data-attached>
        ...
        <div class="overlay">
            <div class="modal">
                ...
                <form class="user" ref="user" :action="`api/users/${slug}`" accept-charset="UTF-8" @submit.prevent="post">
                    <section>
...
<script>
    ...
    export default {
        ...
        methods: {
            post() {
                this.$http.post(`users/${this.slug}`, new FormData(this.$refs.user));
            },

            test() {
//                this.showModal = false;
                console.log('dispatch');
                this.$refs.modal.dispatchEvent(new CustomEvent('hidepopup', {
                    detail: this,
                    bubbles: true,
                    cancelable: true
                }));
        ...
        beforeRouteEnter(to, from, next) {
            Vue.http.get(`/users/${to.params.slug}`).then(response => {
//                response.json().then(data => {
//                    next(vm => vm.$data.user = data);
//                })
                next(vm => vm.$data.user = {});
            });
        },
